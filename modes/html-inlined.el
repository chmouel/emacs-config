;; html-inlined.el -- narrow/widen blocks of php, css, javascript or
...
(defun html-inlined-make-inlined-keymap()
  (let ((m (make-sparse-keymap)))
    (define-key m html-inlined-key 'html-inlined-widen)
...
(defcustom html-inlined-key [(control c) ?%]
  ...
  :set (lambda(symbol value)
         ...
         (html-inlined-make-inlined-keymap)))
           ...
           (handler-list
            (if (re-search-backward html-inlined-start-regexp nil t)
                (loop for x in html-inlined-regions
                      ...
                      finally do (error "html-inlined: regexp mismatch"))
              (message "Not in an inlined region.")
              nil)))
      (if handler-list
          (progn
            (html-inlined-find-indentation (point))
            ;;(html-inlined-narrow-handler orig (cdr handler-list) (match-beginning 0)))
